...
def plot_tokens_by_max(cos_sim, cos_sim_idx, sp_mtrx, users_tokens_df, bow, topTKs: int=20, norm_sp: bool=False):
	...
	for _, usr in enumerate(nUsers_with_max_cosine):
		...
		ax = users_tokens_df.plot.barh(	y=list(users_tokens_df.loc[usr, :].sort_values(ascending=False).index[:topTKs]), 
																		x=users_tokens_df.loc[usr, :].sort_values(ascending=False).values.tolist()[:topTKs],
																		color="#0000ff",
																		height=0.4,
																		)
