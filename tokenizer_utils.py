...
def trankit_lemmatizer(docs):
	...
	#lm = [ re.sub('#', '', tk.get("lemma").lower()) for tk in all_dict.get("tokens") if ( tk.get("lemma") and tk.get("upos") not in useless_upos_tags ) ] 
	...
	all_dict = p(docs)
	lm = [ re.sub('#', '', tk.get("lemma").lower())  for sent in all_dict.get("sentences") for tk in sent.get("tokens") if ( tk.get("lemma") and tk.get("upos") not in useless_upos_tags ) ] 
	#lm = [ tk.get("lemma").lower()  for tk in all_dict.get("tokens") if ( tk.get("lemma") and tk.get("upos") not in useless_upos_tags ) ]
