...
# python user_token.py --inputDF /scratch/project_2004072/Nationalbiblioteket/NLF_DATASET/nikeX.docworks.lib.helsinki.fi_access_log.07_02_2021.log.gz --outDIR /scratch/project_2004072/Nationalbiblioteket/dataframes_x2 --maxNumFeat -1
...
def get_user_df(dframe: pd.DataFrame, bow: Dict[str, int], learned_weights: bool=False):
	...
	try:
		...
		sq_list = df_preprocessed["cleaned_sq_phrase"].map(lambda ph: get_lemmatized_qu_(ph, lm=args.lmMethod) if ph else np.nan, na_action="ignore")
	...
	try:
		...
		sn_list = df_preprocessed['cleaned_search_results_snippets'].map(lambda lst: get_lemmatized_sn(lst, lm=args.lmMethod) if lst else np.nan, na_action='ignore')
	...
	try:
		...
		snHW_list = df_preprocessed["cleaned_search_results_hw_snippets"].map(lambda lst: get_lemmatized_snHWs(lst, lm=args.lmMethod) if lst else np.nan, na_action='ignore')
