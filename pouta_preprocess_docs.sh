...
## $ nohup bash pouta_preprocess_docs.sh 0 > /media/volume/trash/NLF/check_nlf_output.out 2>&1 & # with output saved in check_output.out

## $ nohup parallel -j 50 "bash pouta_preprocess_docs.sh {}" ::: {682..731} > /dev/null 2>&1 &
## $ nohup parallel -j 4 "bash pouta_preprocess_docs.sh {}" ::: {683..708} > /media/volume/trash/NLF/check_nlf_parallel_jobs.out 2>&1 &
