...
## $ nohup parallel -j 300 "bash pouta_preprocess_docs.sh {}" ::: {151..366} > /media/volume/trash/NLF/nlf_preprocess_docs_parallel_jobs_151_366.out 2>&1 &
...
echo "Processing Q[$qIDX]: ${files[$qIDX]}"
	python -u preprocess_documents.py \
		--inputDF ${files[$qIDX]} \
		--outDIR $DATAFRAME_DIR >>$STORAGE_DIR/Nationalbiblioteket/trash/nk_q_$qIDX.out 2>&1 &
else
