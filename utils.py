...
def get_df_pseudonymized_logs(infile="", TIMESTAMP=None):
	...
	# print("<>"*50)
	th = datetime.timedelta(days=0, minutes=0, seconds=5, )
	# print(th, type(th))
...
def get_spMtx(df: pd.DataFrame, meaningless_lemmas: Set, spm_fname: str="SPM", spm_rows_fname: str="SPM_rows", spm_cols_fname: str="SPM_cols", df_unpacked_fname: str="df_unpacked", meaningless_lemmas_zero_nlf_pages_fname: str="meaningless_lemmas_zero_nlf"):
	...
	user_token_df = user_token_df.drop(columns=meaningless_columns_to_be_removed, errors='ignore')
	# ##################################################################################################################
	...
	columns_with_certain_suffixes_2_be_removed = [
		...
		if re.search(
			r"(musiker|museums|museet|museets|musikaliska|historiska|tehtaita|tehd|kokoelmat|skolans|skolan|tehda|tehta|tehtaa|tehtaat|tehtai|nens|bbens|maskiner|människor|människors|människan|människans|människornas|människorna|stillä|stiltä|stilles|stilla|stillas|stissä|stistä|qvistille|qvistil|qvistin|qvistiin|qvis|qvisa|qvistia|tiä|nders|grams|tens|tiön|renas|frågan|frågans|banken|bankens|banker|römin|nader|naden|nadens|byggnads|tröms|bolags|bolaget|bolagets|akatemias|sonia|yrket|filmer|yrkets|vänner|vännen|kvällar|kvällen|kvällens|dagl|tjänster|kurser|gatta|gatas|gatorna|gator|gatare|siona|sions|sionen|sioner|karen|rens|tts|tens|läk|rias|riker|rikens|riks|möt|möter|kyrkan|hyrans|hyran|huset|husets|hemmet|hemmets|hems|ters|handl|hamns|hamna|kvinn|pojkens|pojkar|pojkars|kvinnan|kvinnans|kvinnor|litets|manista|manille|manilla|manill|manilta|manilt|mania|mans|manin|dagar|dagars|dagen|dagens|dagens|daga|agaren|agarens|agares|borga|borgs|borge|dotters|dott|skolan|skolans|skolor|skolorna|skolornas|skillnaden|erna|ernas|garna|garnas|skillnader|heten|heter|hetens|avgiften|avgiftens|avgifter|kaupungissa|kaupungista|kaupungit|gatans|bergs|tionen|tioner|tionens|statio|tiona|tions|tionerne|tione|ingen|ingana|regeringa|ingaina|ingania|ingens|ingene|ingar|ringarne|systems|principen|principer|bergin|gatan|grens|sons|lunde|lunda|lunds|lundille|lundilta|lundilla|lundiin|lundia|lundin|lundissa|holms|mens|holme|stads|stadens)$", 
			col
		)
	]
	print(
	...
	try:
		...
		print(f"\t>>> returning {len(user_token_df.columns)} available TOKENs (column(s)) in NLF collection | Elapsed_t: {time.time()-ttt:.2f} sec")
	except Exception as e:
...
def get_idfed_users_norm(spMtx, idf_vec, exponent: float=1.0, save_dir: str="savin_dir", prefix_fname: str="file_prefix"):
	...
	idf_weighted_spMtx = spMtx.multiply(idf_squeezed)
	# Apply the exponent
	if exponent != 1.0:
